...
SampleFeature_original=np.loadtxt('Feature(Dataset3).txt',dtype=np.float64)
SampleFeature=np.array(SampleFeature_original)
SampleFeature=minmax_scale(SampleFeature)

SampleLabel_original=np.loadtxt('Label(Dataset3).txt',dtype=np.int)
sample_label=[]
...
sample_data_old=SampleFeature.reshape(3384,3,64)
...
for i,(train_index,test_index) in enumerate(kf.split(sample_data)):
    ...
    np.savetxt('Feature(Dataset3)(test)'+str(i+1)+'.txt',save_x_test,fmt='%f',delimiter=' ')
    save_y_test=np.array(y_test_temp)
    np.savetxt('Label(Dataset3)(test)'+str(i+1)+'.txt',save_y_test,fmt='%f',delimiter=' ')
    ...
    model.save('CapsNet-LDA_model'+str(i+1)+'(Dataset3).h5')
...
np.savetxt('CapsNet-LDA_fpr(Dataset3)(5-CV).txt',save_fpr,fmt='%f',delimiter=' ')
save_tpr=np.array(mean_tpr)
np.savetxt('CapsNet-LDA_tpr(Dataset3)(5-CV).txt',save_tpr,fmt='%f',delimiter=' ')
...
plt.savefig('auc(CapsNet-LDA)(Dataset3).png', dpi=300)
plt.show()
...
np.savetxt('CapsNet-LDA_precision(Dataset3)(5-CV).txt',save_precision,fmt='%f',delimiter=' ')
save_recall=np.array(recall)
np.savetxt('CapsNet-LDA_recall(Dataset3)(5-CV).txt',save_recall,fmt='%f',delimiter=' ')
...
plt.savefig('aupr(CapsNet-LDA)(Dataset3).png', dpi=300)
plt.show()
